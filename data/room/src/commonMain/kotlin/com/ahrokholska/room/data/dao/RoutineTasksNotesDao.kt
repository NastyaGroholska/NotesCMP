...
import com.ahrokholska.room.data.intermediate.RoutineTasksNoteDetails
import com.ahrokholska.room.data.intermediate.RoutineTasksNoteEntityWithSubNotes
import kotlinx.coroutines.flow.Flow

@Dao
internal abstract class RoutineTasksNotesDao {
    @Query("DELETE FROM routine_tasks_note WHERE id = :noteId")
    abstract suspend fun deleteRoutineTasksOnly(noteId: Int)

    @Insert
    protected abstract suspend fun insertRoutineTasksNote(note: RoutineTasksNoteEntity): Long

    @Insert
    protected abstract suspend fun insertRoutineTasksNoteSubNote(item: RoutineTasksNoteSubNoteEntity)

    @Query("SELECT * FROM routine_tasks_note WHERE rowid=:rowId")
    protected abstract suspend fun getRoutineTasksNoteByRowId(rowId: Long): RoutineTasksNoteEntity

    @Transaction
    open suspend fun insert(note: Note.RoutineTasks) {
    ...
    abstract suspend fun changeRoutineTasksSubNoteCheck(id: Int, index: Int, finished: Boolean)
